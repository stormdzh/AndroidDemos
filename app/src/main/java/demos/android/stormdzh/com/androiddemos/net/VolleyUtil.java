...
public class VolleyUtil {
    ...
    public static void postNet(Context context, String tag, final HashMap<String, String> headers, String url, final TreeMap<String,String> params, final OnNetListener netListener) {
        //1.创建出请求队列
        RequestQueue mRequestQueue = Volley.newRequestQueue(context);
         ...
         * 6p:失败的监听，失败在这里处理
         */
        StringRequest mStrReq = new StringRequest(Request.Method.POST, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        if (netListener != null)
